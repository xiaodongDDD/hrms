...
var H5lock = function (obj) {//初始化H5加锁对象
  ...
  this.outFillStyle = obj.outFillStyle || 'rgba(118, 197, 255, .8)';
  ...
  this.haveDelta= true || obj.haveDelta;//箭头
  this.outFill = true || obj.outFill; //是否填充外层圆圈
};
...
H5lock.prototype.fillOutPoint = function (x, y) {                       //填充外层圆圈方法
  this.ctx.beginPath();
  this.ctx.fillStyle = this.outFillStyle;
...
H5lock.prototype.update = function (po) {                                         //核心变换方法在touchmove时候调用
  ...
  for( i = 0; i < this.lastPoint.length - 1; i++){                                //每帧画三角心
    this.drawTriangle(this.lastPoint[i], this.lastPoint[i + 1]);
  }
  ...
  for (i = 0; i < this.restPoint.length; i++) {                              //更新的时候判断移动到的点是否在剩余节点数组里面
    ...
    if ( getDis(po, pt) < ( this.r * 1.2 ) ) {
      this.drawPoint();                                                   //如果来到一个新的圆圈，我们需要把圆心画出来
      ...
      if( self.operation == self.INIT_PASSWORD){                //初试话密码
        ...
        if(self.step == 0){
          if(self.checkPass(self.password, self.lastPoint)){
            self.step += 1;
            self.setDesc('请设置您的新手势密码');
          } else {
            self.setDesc('手势密码不正确，请重新输入');
            self.errorDraw();
            self.errorCallback();
          }
        } else if (self.step == 1) {
