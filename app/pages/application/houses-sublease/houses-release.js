...
angular.module('applicationModule')
  .controller('HousesReleaseCtrl', [
    ...
    function ($scope,
      ...
      $scope.cityList = [];
      angular.forEach($scope.province_enum[0].city, function (data, index, array) {//初始化默认第一个
        $scope.cityList.push(array[index])
      });
      function showModal() {
        $ionicModal.fromTemplateUrl('build/pages/application/houses-sublease/modal/houses-release-city-choose.html', {//定义modal
          scope: $scope
      ...
      $ionicModal.fromTemplateUrl('build/pages/application/houses-sublease/modal/houses-release-city-area-choose.html', {//定义modal
        ...
        $scope.chooseCityAreaPopup = modal2;
      });//初始化选择城市区域的modal
      ...
      $scope.chooseCity = function () {//城市选择
        $scope.chooseCityPopup.show();
      };
      $scope.chooseCityArea = function () {//城市区域选择
        $scope.chooseCityAreaPopup.show();
      };
      ...
      $scope.finishChoosingCity = function (city) {
        $scope.housesReleaseInfo.city = city.name;
        $scope.chooseCityPopup.hide();
        //showModal();
        $scope.cityAreaList = [];
        $scope.housesReleaseInfo.area = "";
        for (var i = 0; i < $scope.cityList.length; i++) {
          if ($scope.cityList[i].name == city.name) {
            angular.forEach($scope.cityList[i].district, function (data, index, array) {
              $scope.cityAreaList.push(array[index])
            })
          }
        }
      };

      $scope.finishChoosingCityArea = function (cityArea) {
        $scope.housesReleaseInfo.area = cityArea.name;
        $scope.chooseCityAreaPopup.hide();
      };
