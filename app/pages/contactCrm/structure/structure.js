...
angular.module('myApp')
  .config(['$stateProvider',
    function ($stateProvider) {
      $stateProvider
        .state('tab.contactCrmStructure', {
          url: '/contactCrm/contactCrmStructure',
          views: {
            'tab-contactCrm': {
              templateUrl: 'build/pages/contact/structure/structure.html',
              controller: 'structureCrmCtl'
            }
          },
          params: {
            routeId: "1",
      ...
      $scope.nextStructure = function (newDepartmentId) { //到下一级组织架构界面
        try {
          ...
          if ($scope.childrenDept.length != 0 || $scope.childrenDept.length) {
            $state.go("tab.contactCrmStructure" + curr_page, {
              routeId: ++curr_page,
  ...
  .config(['$stateProvider', 'structureDelegateProvider',
    function ($stateProvider, structureDelegateProvider) {
      for (var i = 1; i < structureDelegateProvider._id; i++) { //循环路由
        $stateProvider.state('tab.contactCrmStructure' + i, {
          url: '/contactCrm/contactCrmStructure/' + i,
          views: {
            'tab-contactCrm': {
              templateUrl: 'build/pages/contactCrm/structure/structure.html',
              controller: 'structureCrmCtl'
            }
          },
          params: {
