...
public class CalendarPickerView extends ListView {
  ...
  private DateFormat YYYYMMDDFormat;
  ...
  public CalendarPickerView(Context context, AttributeSet attrs) {
    ...
    TypedArray a = context.obtainStyledAttributes(attrs, getStyleableArryId("CalendarPickerView"));
    final int bg = a.getColor(getStyleableId("CalendarPickerView_android_background"),
        res.getColor(getColorId("calendar_bg")));
    dividerColor = a.getColor(getStyleableId("CalendarPickerView_tsquare_dividerColor"),
        res.getColor(getColorId("calendar_divider")));
    dayBackgroundResId = a.getResourceId(getStyleableId("CalendarPickerView_tsquare_dayBackground"),
        getDrawableId("calendar_bg_selector"));
    dayTextColorResId = a.getResourceId(getStyleableId("CalendarPickerView_tsquare_dayTextColor"),
            getColorId("calendar_text_selector"));
    titleTextColor = a.getColor(getStyleableId("CalendarPickerView_tsquare_titleTextColor"),
            res.getColor(getColorId("calendar_text_head")));
    displayHeader = a.getBoolean(getStyleableId("CalendarPickerView_tsquare_displayHeader"), true);
    headerTextColor = a.getColor(getStyleableId("CalendarPickerView_tsquare_headerTextColor"),
            res.getColor(getColorId("calendar_text_head")));
    a.recycle();
    ...
    monthNameFormat = new SimpleDateFormat(context.getString(getStringId("month_name_format")), locale);
    weekdayNameFormat = new SimpleDateFormat(context.getString(getStringId("day_name_format")), locale);
    YYYYMMDDFormat= new SimpleDateFormat(context.getString(getStringId("YYYY_MM_DD_format")), locale);
  ...
  public FluentInitializer init(Date minDate, Date maxDate, Locale locale) {
    ...
    monthCounter = Calendar.getInstance(locale);
    monthNameFormat =
        new SimpleDateFormat(getContext().getString(getStringId("month_name_format")), locale);
    for (MonthDescriptor month : months) {
      month.setLabel(monthNameFormat.format(month.getDate()));
    }
    weekdayNameFormat =
        new SimpleDateFormat(getContext().getString(getStringId("day_name_format")), locale);
    fullDateFormat = DateFormat.getDateInstance(DateFormat.MEDIUM, locale);
  ...
  public class FluentInitializer {
    ...
    public FluentInitializer setShortWeekdays(String[] newShortWeekdays) {
      ...
      weekdayNameFormat =
          new SimpleDateFormat(getContext().getString(getStringId("day_name_format")), symbols);
      return this;
    }
...
//      }
  ...
  private class MonthAdapter extends BaseAdapter {
    ...
    @Override public View getView(int position, View convertView, ViewGroup parent) {
      ...
      if (monthView == null
             || !monthView.getTag(getId("day_view_adapter_class")).equals(dayViewAdapter.getClass())) {
        monthView =
        ...
        monthView.setTag(getId("day_view_adapter_class"), dayViewAdapter.getClass());
      } else {
...
//            isCurrentMonth && betweenDates(cal, minCal, maxCal) && isDateSelectable(date);
        ...
        int valuemonth =cal.get(MONTH);
        int valueyear =cal.get(YEAR);
        ...
        weekCells.add(
            new MonthCellDescriptor(date, isCurrentMonth, isSelectable, isSelected, isToday,
                isHighlighted, value,valuemonth,valueyear, rangeState));
        cal.add(DATE, 1);
      }
    }
    return cells;
  ...
  private class DefaultOnInvalidDateSelectedListener implements OnInvalidDateSelectedListener {
    @Override public void onInvalidDateSelected(Date date) {
      String errMessage =
          getResources().getString(getStringId("invalid_date"), fullDateFormat.format(minCal.getTime()),
...
//                    Log.e("399", fields[j].getName());
  ...
  private int getStringId(String stringId){
    return mContext.getResources().getIdentifier(stringId, "string", mContext.getPackageName());
  }
