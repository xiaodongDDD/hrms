...
//  youtuiShareDemo
//
//  Created by FreeGeek on 15-1-30.
...
static const char kKeyChainUDIDAccessGroup[] = "http://youtui.mobi";

@implementation FreeGeekCode

+ (NSString*)Code
{
    NSString *udid = [FreeGeekCode getUDIDFromKeyChain];
    if (!udid) {
        ...
        if (version >= 7.0) {
            udid = [FreeGeekCode iOS7];
        }
        else if (version >= 2.0) {
            udid = [FreeGeekCode iOS6];
        }
        
        [FreeGeekCode settUDIDToKeyChain:udid];
    }
...
+ (NSString*)iOS6
{
    return [FreeGeekCode getMacAddress];
}
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([FreeGeekCode getUDIDFromKeyChain]) {        // there is item in keychain
        [FreeGeekCode updateUDIDInKeyChain:udid];
        return YES;
    }
