...
@property (nonatomic,strong)WXTramsitionPresentMode *presentMode;
@property (nonatomic,strong)WXTramsitionPresentMode *groupPresentMode;
@end
...
{
    NSString *businessUrl = [command.arguments[0] objectForKey:@"businessUrl"];
    ...
    NSLog(@"%@,%@,%@,%@",businessUrl,userId,access_token,Token);
    
    [[NSUserDefaults standardUserDefaults] setObject:businessUrl forKey:@"businessUrl"];
    ...
    [self performSelector:@selector(IMPluginDidReceiveMessage:) withObject:nil afterDelay:1.0];
}
...
- (void)toChatAct:(CDVInvokedUrlCommand *)command
{
    ...
    if (isSuccessFulConnect==ConnectionStatus_Connected) {
        ...
        chatVC.target_id = friendId;
        chatVC.friendIcon = friendIcon;
        chatVC.navtitle = friendName;
        chatVC.phoneNums = telephones;
        
        self.presentMode = [[WXTramsitionPresentMode alloc]initWithPresented:^(UIViewController *presented, UIViewController *presenting, UIViewController *source, WXTransationAnimation *transition) {
            transition.transitionMode = kWXTtransitionPresented;
        } dismissed:^(UIViewController *dismissed, WXTransationAnimation *transition) {
            transition.transitionMode = kWXTtransitionDismissed;
        }];
        
        UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:chatVC];
        nav.transitioningDelegate = self.presentMode;
        [self.viewController presentViewController:nav animated:YES completion:NULL];
    }else{
        //[ToastUtils showLong:@"登录聊天服务器失败,请确保您的网络可用或重新登录"];
        NSString *text ;
        switch (isSuccessFulConnect) {
            case ConnectionStatus_Connecting:
                text = @"网络连接状态：连接中，请稍后";
                break;
            case ConnectionStatus_Unconnected:
                text = @"网络连接状态：连接失败或未连接";
                break;
            case ConnectionStatus_TOKEN_INCORRECT:
                text = @"网络连接状态：Token无效,请重新登录";
                break;
            case ConnectionStatus_SignUp:
                text = @"网络连接状态：已注销";
                break;
            case ConnectionStatus_KICKED_OFFLINE_BY_OTHER_CLIENT:
                text = @"网络连接状态：当前用户在其他设备上登陆";
                break;
            default:
                text = [NSString stringWithFormat:@"网络连接状态码:%li",isSuccessFulConnect];
                break;
        }
...
- (void)openDiscussion:(CDVInvokedUrlCommand *)commmand
{
    ...
    self.groupPresentMode = [[WXTramsitionPresentMode alloc]initWithPresented:^(UIViewController *presented, UIViewController *presenting, UIViewController *source, WXTransationAnimation *transition) {
        transition.transitionMode = kWXTtransitionPresented;
    } dismissed:^(UIViewController *dismissed, WXTransationAnimation *transition) {
        transition.transitionMode = kWXTtransitionDismissed;
    }];
    ...
    nav.transitioningDelegate = self.groupPresentMode;
...
- (void)createDiscussion:(CDVInvokedUrlCommand *)command
{
    ...
    RCIMDiscussionGroupAddVC.isCreated = YES;
    ...
    for (RCConversation * conversation in conversationList) {
        ...
        if (conversation.conversationType==ConversationType_PRIVATE) {
            ...
            if ([conversation.objectName isEqualToString:@"RC:TxtMsg"]) {
                ...
                RCDiscussionNotificationMessage *RCDiscussionNotiMsg = (RCDiscussionNotificationMessage*)conversation.lastestMessage;
                switch (RCDiscussionNotiMsg.type) {
                    case RCInviteDiscussionNotification:
                        content = [NSString stringWithFormat:@"%@邀请了%@加入了讨论组",RCDiscussionNotiMsg.operatorId,RCDiscussionNotiMsg.extension];
                        break;
                    case RCQuitDiscussionNotification:
                        content = [NSString stringWithFormat:@"%@退出了讨论组",RCDiscussionNotiMsg.operatorId];
                        break;
                    case RCRenameDiscussionTitleNotification:
                        content = [NSString stringWithFormat:@"%@修改了讨论组名称",RCDiscussionNotiMsg.operatorId];
                        break;
                    default:
                        break;
                }
            ...
            icon = @"http://zhouzybk.img-cn-shanghai.aliyuncs.com/group_icon_default1476255700792.png";
...
- (void)requestUserNameAndUrlById:(NSString *)userId ByToken:(NSString *)access_token
{
    ...
    NSURLSessionDataTask *Task = [session dataTaskWithRequest:mutableRequest completionHandler:^(NSData * data, NSURLResponse *  response, NSError * error) {
        ...
        if (!error) {
            ...
            NSLog(@"userId:%@,userIcon:%@,userName:%@,tel:%@",userId,userIcon,userName,tel);
            ...
            if (json[@"error"]) {
                ...
                NSLog(@"下载头像成功:%@, 姓名：%@ ：电话:%@",userIcon,userName,tel);
