...
-(void)viewDidLoad
{
    ...
    NSLog(@"讨论组的头像:%@,id:%@",self.discussionImageName,self.discussionId);
...
- (void)discussionSetting
{
    ...
    RCIMDiscussionSettingVC.members = [NSMutableArray arrayWithArray:self.membersList];
...
- (void)updateHistoryMessage:(UIRefreshControl *)control
{
    ...
    NSLog(@"historyArray：%li",historyArray.count);
...
- (void)didReceivedMessageNotification:(NSNotification *)notification
{
    ...
    //主要判断条件
    if ([message.targetId isEqualToString:self.discussionId]) {
        RCIMDiscussionMessageFrame *messageFrame = [[RCIMDiscussionMessageFrame alloc] init];
    ...
    RCDiscussionNotificationMessage *discussionMessage;
    if ([message.content isKindOfClass:[RCDiscussionNotificationMessage class]]) {
        discussionMessage = (RCDiscussionNotificationMessage *)message.content;
        NSLog(@"讨论组界面收到消息了:%@ 类型：%li 目标：%@",discussionMessage,discussionMessage.type,message.targetId);
    }else if ([message.content isKindOfClass:[RCTextMessage class]]){
    
        RCTextMessage *textMessage = (RCTextMessage *)message.content;
        NSLog(@"讨论组界面wenben:%@",textMessage.content);
    }
