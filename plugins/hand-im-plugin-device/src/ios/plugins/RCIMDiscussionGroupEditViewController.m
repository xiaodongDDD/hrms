...
//                NSLog(@"self.memberList:%@",self.memberList);
                //把创建者也加入进去
                NSString *creatorId = [[NSUserDefaults standardUserDefaults] objectForKey:@"userId"];
                NSString *creatorName = [[NSUserDefaults standardUserDefaults] objectForKey:@"userName"];
                NSString *creatorIcon = [[NSUserDefaults standardUserDefaults]objectForKey:@"userIcon"];
                NSDictionary *creatorInfo = [NSDictionary dictionaryWithObjects:@[creatorId,creatorName,creatorIcon] forKeys:@[@"emp_code",@"emp_name",@"avatar"]];
                NSMutableArray *array = [NSMutableArray arrayWithArray:self.memberList];
                [array insertObject:creatorInfo atIndex:0];
                RCIMGroupChattingVC.membersList = array;//info
                ...
                if ([self.imageView.image isEqual:[UIImage imageNamed:@"add_members@2x.png"]]) {
                    [self upload:@"discussionGroupImage.png" mimeType:@"image/jpeg" fileData:UIImageJPEGRepresentation([self imageWithTitle:[self firstCharactorWithString:self.textField.text] Size:CGSizeMake(80, 80)], 1.0) params:nil];
                }else{
