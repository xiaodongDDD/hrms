...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    //        if (![messsage.objectName isEqualToString:@"RC:VCSummary"]) {
    ...
    for (int i=array.count-1; i>=0; i--) {
        RCMessage *messsage  = [array objectAtIndex:i];
        MessageFrame *msgFrame = [[MessageFrame alloc] init];
        msgFrame.message = messsage;
        if (![messsage.objectName isEqualToString:@"RC:VCSummary"]) {
            [self.dataSource addObject:msgFrame];
...
- (void)setUI
{   //聊天界面
    _ChatTableView = [[ChattingTableView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, self.view.bounds.size.height-80-64) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==0) {
        ...
        //显示时间控制在5分钟之内
        MessageFrame *messageFrame1 = _dataSource[indexPath.row-1];
...
- (void)call
{
    ...
    [alertVC addAction:[UIAlertAction actionWithTitle:@"网络电话" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
        RCConnectionStatus connectStatus = [[RCIM sharedRCIM] getConnectionStatus];
        switch (connectStatus) {
                //连接状态
            case ConnectionStatus_Connected:{
                //网络电话
#pragma mark CallKit ios 10 =============
                if([[UIDevice currentDevice]systemVersion].floatValue<10.0f){
                    [[RCCall sharedRCCall] startSingleCall:self.target_id
                                                 mediaType:RCCallMediaAudio];
                    NSString *path = [[[NSBundle mainBundle] pathForResource:@"RongCloud" ofType:@"bundle"] stringByAppendingPathComponent:@"voip/voip_calling_ring.mp3"];
                    NSLog(@"mp3path:%@",path);
                }else{
                    [[CustomRCCall sharedRCCall] startSingleCall:self.target_id mediaType:RCCallMediaAudio];
                }
                break;
            }
            ...
            case ConnectionStatus_KICKED_OFFLINE_BY_OTHER_CLIENT:{
                [ToastUtils showLong:@"当前帐号已在其他设备登录!"];
                break;
            }
            default:
                [ToastUtils showLong:@"未知状态,请检查网络或者帐号已登录"];
                break;
        }
...
- (void)didTapAction:(ChatCell *)cell
{
    if ([cell.msgFrame.message.content isKindOfClass:[RCTextMessage class]]) {
        ...
        if(imageMessage.originalImage){
            imageView = [[UIImageView alloc] initWithImage:imageMessage.originalImage];
        }else{
