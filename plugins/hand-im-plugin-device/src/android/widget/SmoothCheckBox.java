...
public class SmoothCheckBox extends View implements Checkable {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = getContext().obtainStyledAttributes(attrs, getStyleableArryId("SmoothCheckBox"));
        int tickColor = ta.getColor(getStyleableId("SmoothCheckBox_color_tick"), COLOR_TICK);
        mAnimDuration = ta.getInt(getStyleableId("SmoothCheckBox_duration"), DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(getStyleableId("SmoothCheckBox_color_unchecked_stroke"), COLOR_FLOOR_UNCHECKED);
        mCheckedColor = ta.getColor(getStyleableId("SmoothCheckBox_color_checked"), COLOR_CHECKED);
        mUnCheckedColor = ta.getColor(getStyleableId("SmoothCheckBox_color_unchecked"), COLOR_UNCHECKED);
        mStrokeWidth = ta.getDimensionPixelSize(getStyleableId("SmoothCheckBox_stroke_width"), CompatUtils.dp2px(getContext(), 0));
        ta.recycle();
    ...
    public void setCanClick(boolean canClick){//设置是否能够点击
...
//                mScaleVal = (float) animation.getAnimatedValue();
                mScaleVal = Float.valueOf(animatedValue.toString());
...
//                mFloorScale = (float) animation.getAnimatedValue();
                mFloorScale = Float.valueOf(animatedValue.toString());
                postInvalidate();
            }
        });
        floorAnimator.start();
